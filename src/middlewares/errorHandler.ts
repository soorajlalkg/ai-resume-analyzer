...
export function errorHandler(
  ...
  if (err instanceof QueryFailedError) {
    const message = process.env.NODE_ENV === 'production' ? 'Database query failed' : err.message;
  ...
  if (err instanceof Error && 'name' in err) {
    ...
    if (['TokenExpiredError', 'JsonWebTokenError', 'NotBeforeError'].includes(errorWithName.name)) {
      return res.status(401).json(failedResponse(errorWithName.message, 401, errorWithName.name));
    }
  }
